 ...
 */
function createChainHash(eventHash: string, previousHash: string, index: number): string {
...
export function verifyChainIntegrity(): {
  ...
  for (let i = 0; i < hashChain.length; i++) {
    ...
    // Verify chain hash
    const expectedChainHash = createChainHash(entry.eventHash, entry.previousHash, i);
  ...
  if (!valid) {
    emitLog(`[TraceImmutability] 🚨 INTEGRITY VIOLATION DETECTED: ${errors.length} errors`);
  }
 ...
 */
export async function verifyTraceEntry(traceId: string, index: number): Promise<{
  valid: boolean;
  error?: string;
}> {
  const entry = hashChain.find(e => e.traceId === traceId && e.index === index);
...
export function getChainStats(): {
  ...
  return {
    ...
    lastHash,
    lastTimestamp: hashChain.length > 0 ? hashChain[hashChain.length - 1].timestamp : null
  };
}
...
export function createSnapshot(): {
  ...
  emitLog(`[TraceImmutability] 📸 Snapshot created: ${snapshotHash.substring(0, 16)}...`);
