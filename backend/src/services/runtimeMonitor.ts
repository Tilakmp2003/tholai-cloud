...
export class RuntimeMonitorService {
  ...
  async startMonitoring(projectId: string, runCommand: string = 'npm start') {
    ...
    const stopExec = await sandbox.streamExec(containerId, runCommand, (log, type) => {
  ...
  private async analyzeLog(projectId: string, log: string) {
    ...
    if (isError) {
      console.log(`[RuntimeMonitor] 🚨 Detected Error in ${projectId}:`, log.slice(0, 100));
  ...
  private async triggerRepair(projectId: string, errorLog: string) {
    ...
    // 1. Map Stack Trace
    const mappedSymbol = await stackTraceMapper.mapToSymbol(projectId, errorLog);
    let context = '';
    
    if (mappedSymbol) {
      console.log(`[RuntimeMonitor] 📍 Mapped to ${mappedSymbol.file.path}`);
      const symbolDetails = await repoQuery.fetchBySymbol(projectId, mappedSymbol.symbol.name);
      if (symbolDetails) {
        context = `\nContext:\n${symbolDetails.code}`;
      }
    }
    ...
    try {
      ...
      const task = await prisma.task.create({
        data: {
          title: `Fix Runtime Error: ${errorLog.split('\n')[0].slice(0, 50)}...`,
