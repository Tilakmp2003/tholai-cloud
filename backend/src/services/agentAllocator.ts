...
export async function analyzeProject(prd: string): Promise<ProjectAnalysis> {
  ...
  try {
    const response = await callLLM(await getAgentConfig("TeamLead"), [
      {
        role: "system",
        content:
          "You are a project analyzer. Extract metrics from PRD and return ONLY valid JSON. No markdown, no extra text.",
      },
      {
        role: "user",
 ...
 * SIMPLIFIED: Smaller teams for simple projects
 */
 ...
 */
function augmentForDynamicMetrics(
  baseComp: AgentAllocation,
  metrics: { workflowsPerHour: number; complexity: number }
): AgentAllocation {
  const comp = { ...baseComp };
 ...
 */
export async function allocateAgentsForProject(
  projectId: string,
  prdText: string
) {
  ...
  if (
    lastAlloc &&
    Date.now() - new Date(lastAlloc.createdAt).getTime() < COOLDOWN_MS
  ) {
  ...
  if (totalAgents > MAX_AGENTS) {
    console.warn(
      `[AgentAllocator] Allocation ${totalAgents} exceeds MAX ${MAX_AGENTS}. Scaling down not fully implemented, relying on budget check.`
    );
  }
  if (totalAgents < MIN_AGENTS) {
    console.warn(
      `[AgentAllocator] Allocation ${totalAgents} below MIN ${MIN_AGENTS}.`
    );
  ...
  if (estimatedDailyCost > DAILY_BUDGET_USD) {
    console.warn(
      `[AgentAllocator] Budget breach: $${estimatedDailyCost.toFixed(
        2
      )} > $${DAILY_BUDGET_USD}. Scaling down.`
    );
  ...
  console.log(
    `[AgentAllocator] Allocation successful. Total agents: ${getTotalAgentCount(
      desiredComposition
    )}. Cost: $${estimatedDailyCost.toFixed(2)}`
  );
