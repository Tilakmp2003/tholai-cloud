...
export class WorkspaceManager {
  ...
  async initializeWorkspace(projectId: string, projectName: string): Promise<string> {
    ...
    try {
      ...
      console.log(`[Workspace] ⚠️  Workspace already exists at: ${projectPath}`);
...
`;
  ...
  async writeFile(projectId: string, relativePath: string, content: string): Promise<void> {
    ...
    console.log(`[Workspace] 💾 Wrote file: ${relativePath} (${content.length} bytes)`);
  }
  ...
  async startPreview(projectId: string): Promise<number> {
    ...
    try {
      ...
      console.log(`[Workspace] ⚠️  npm install skipped or failed (may already be installed)`);
    ...
    // Handle process exit
    child.on('exit', (code) => {
      console.log(`[Workspace] 🛑 Dev server on port ${port} exited with code ${code}`);
    ...
    console.log(`[Workspace] ✅ Dev server started on http://localhost:${port}`);
  ...
  async runCommand(projectId: string, command: string): Promise<{ stdout: string; stderr: string; exitCode: number }> {
    ...
    console.log(`[Workspace] 🏃 Running command: "${command}" in ${project.workspacePath}`);
