...
export class GitIntegrationService {
  ...
  async initRepo(workspacePath: string, projectName: string): Promise<boolean> {
    try {
      ...
      await execAsync('git config user.email "ai-agent@virtualcompany.dev"', {
        cwd: workspacePath,
      });
      await execAsync('git config user.name "AI Agent"', {
        cwd: workspacePath,
      });
...
`;
      ...
      await execAsync(`git commit -m "🎉 Initial commit: ${projectName}"`, {
        cwd: workspacePath,
      });
  ...
  async createBranch(
    ...
    try {
      ...
      try {
        await execAsync(`git rev-parse --verify ${safeBranch}`, {
          cwd: workspacePath,
        });
        ...
        await execAsync(`git checkout -b ${safeBranch}`, {
          cwd: workspacePath,
        });
  ...
  async commitTaskChanges(
    ...
    try {
      ...
      if (gate.status === "PENDING") {
        ...
        try {
          const resolvedGate = await approvalGates.waitForApproval(
            gate.id,
            300000
          ); // 5 min timeout
          ...
          if (resolvedGate.status === "REJECTED") {
            ...
            return {
              success: false,
              error: `Rejected: ${resolvedGate.reviewerNotes}`,
            };
          }
        } catch (timeoutError) {
      ...
      const agentInfo = task?.assignedToAgent
        ? `\n\nAgent: ${
            task.assignedToAgent.role
          } (${task.assignedToAgent.id.substring(0, 8)})`
        : "";
  ...
  async rollback(workspacePath: string, commitHash: string): Promise<boolean> {
    try {
      await execAsync(`git revert ${commitHash} --no-commit`, {
        cwd: workspacePath,
      });
      await execAsync(`git commit -m "⏪ Rollback: Revert ${commitHash}"`, {
        cwd: workspacePath,
      });
  ...
  async getBranches(workspacePath: string): Promise<BranchInfo[]> {
    try {
      ...
      const { stdout: branches } = await execAsync("git branch -v", {
        cwd: workspacePath,
      });
