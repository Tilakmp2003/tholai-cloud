...
export interface MemoryEntry {
  ...
  agentId?: string;
  agentRole: string;
  type: 'SUCCESS_PATTERN' | 'FAILURE_LESSON' | 'DOMAIN_KNOWLEDGE' | 'CODE_SNIPPET' | 'BEST_PRACTICE';
...
async function extractLearnings(
  task: any,
  success: boolean
): Promise<Array<{ type: MemoryEntry['type']; title: string; content: string; category: string }>> {
  const prompt = `Analyze this ${success ? 'successful' : 'failed'} task and extract reusable learnings.
...
export async function learnFromTask(
  ...
  emitLog(`[AgentMemory] 📚 Learning from ${success ? 'successful' : 'failed'} task...`);
...
export async function retrieveRelevantMemories(
  ...
  const queryText = `${taskContext.summary || ''} ${taskContext.description || ''} ${JSON.stringify(taskContext)}`;
 ...
 */
export function updateMemoryOutcome(memoryId: string, wasSuccessful: boolean): void {
  const entry = memoryStore.get(memoryId);
  if (!entry) return;
  
  // Exponential moving average
  const alpha = 0.3;
  entry.successRate = alpha * (wasSuccessful ? 1 : 0) + (1 - alpha) * entry.successRate;
  entry.updatedAt = new Date();
...
export function getMemoryStats(): {
  ...
  return {
    ...
    byType,
    byCategory,
    avgSuccessRate: memories.length > 0 ? totalSuccessRate / memories.length : 0
  };
}
