...
export class GitHubService {
  ...
  private async commitModule(projectName: string, moduleName: string, tasks: any[]) {
  ...
  private async commitTask(branchName: string, projectName: string, moduleName: string, task: any, taskNum: number) {
    ...
    const filePath = `${this.sanitize(projectName)}/${this.sanitize(moduleName)}/${fileName}`;
    ...
    try {
      ...
      await this.octokit.repos.createOrUpdateFileContents({
        ...
        repo: this.repo,
        path: filePath,
        message: `🤖 Add ${fileName}\n\nGenerated by: ${task.assignedToAgent?.role || 'Agent'}\nTask ID: ${task.id}`,
  ...
  private createPRDescription(projectName: string, moduleName: string, tasks: any[]): string {
...
${tasks.map((t, i) => `- \`task_${i + 1}_${t.id.substring(0, 8)}.ts\``).join('\n')}

### Agents Involved

${Array.from(new Set(tasks.map(t => t.assignedToAgent?.role).filter(Boolean))).map(role => `- ${role}`).join('\n')}
...
`;
  ...
  private extractCode(result: any): string | null {
    ...
    if (result.code) return typeof result.code === 'string' ? result.code : JSON.stringify(result.code, null, 2);
    if (result.implementation) return typeof result.implementation === 'string' ? result.implementation : JSON.stringify(result.implementation, null, 2);
...
if (require.main === module) {
  ...
  if (!projectName) {
    console.error('❌ Usage: npx tsx src/github/githubService.ts "Project Name"');
    process.exit(1);
  }
  ...
  service.commitGeneratedCode(projectName)
    .then(() => console.log('\n🎉 Done!'))
