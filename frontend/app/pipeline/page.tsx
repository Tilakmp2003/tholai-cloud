...
const COLUMN_STYLES: Record<
  string,
  { bg: string; gradient: string; glow: string; text: string; border: string }
> = {
  zinc: {
    bg: "bg-zinc-500",
...
export default function PipelinePage() {
  ...
  const { data: tasks, isLoading } = useProjectTasks(selectedProjectId);
  const [selectedTask, setSelectedTask] = useState<any>(null);
  const containerRef = useRef<HTMLDivElement>(null);

  // Handle new project creation - auto-switch to it
  const handleNewProject = useCallback((newProject: any) => {
    console.log(
      "[Pipeline] Switching to newly created project:",
      newProject.id
    );
    setSelectedProjectId(newProject.id);
  }, []);

  // Register callback for new project events
  useEffect(() => {
    setOnNewProjectCallback(handleNewProject);
    return () => setOnNewProjectCallback(null);
  }, [handleNewProject]);

  // Auto-select the most recent project on first load
  useEffect(() => {
    if (projects.length > 0 && !selectedProjectId) {
      // Sort by createdAt descending and pick first
      const sorted = [...projects].sort(
        (a: any, b: any) =>
          new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      );
      setSelectedProjectId(sorted[0]?.id || null);
    }
  ...
  const activeThreads =
    Object.values(tasks || {})
      .flat()
      .filter((t: any) => typeof t === "object").length || 0;
    ...
    <div className="flex flex-col h-screen bg-zinc-950 text-zinc-50 overflow-hidden selection:bg-indigo-500/30">
      ...
      <header className="flex items-center justify-between px-8 py-6 border-b border-white/5 bg-zinc-950/50 backdrop-blur-md z-10">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            <span className="text-[10px] font-mono text-zinc-500 tracking-widest uppercase">
              ACTIVE THREADS:{" "}
              <span className="text-zinc-300">{activeThreads}</span>
            </span>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <SelectTrigger className="w-[250px] bg-zinc-900/50 border-white/10 text-zinc-300">
...
function PipelineColumn({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "absolute top-1 inset-x-0 h-32 bg-gradient-to-b to-transparent opacity-20 pointer-events-none",
          styles.gradient
        )}
      />
...
function TaskCard({
  ...
  const latestTrace =
    task.traces && task.traces.length > 0 ? task.traces[0] : null;
  const statusMessage = latestTrace
    ? latestTrace.event
    ...
    >
        ...
        {task.priority && (
          <div
            className={cn(
              ...
              task.priority === "HIGH" ? "bg-red-500" : "bg-zinc-600"
            )}
          />
        )}
      </div>
      ...
      <div className="mb-3">
        ...
        <p className="text-xs text-zinc-400 font-mono line-clamp-2">
          {latestTrace
            ? `> ${latestTrace.event}`
            : task.contextPacket?.description || task.status || "Pending..."}
        </p>
      </div>
                ...
                {traces.map((trace, i) => (
                  <div key={i} className="flex gap-3 text-xs font-mono">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-zinc-300">{trace.event}</span>
                      {trace.metadata && (
